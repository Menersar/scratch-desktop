 ...
 * @returns {Promise<ArrayBuffer>}
 */
const requestFromMainThreadAsArrayBuffer = (url) =>
  ipcRenderer.invoke("request-url", url);
...
const fetchScratchProject = async (id) => {
  ...
  try {
    ...
    const projectMetadata = parseBufferAsJSON(
      await ipcRenderer.invoke("get-project-metadata", id)
    );
