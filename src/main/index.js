        ...
        submenu: [
          {
            ...
            click: () =>
              shell.openExternal(
                "https://menersar.github.io/Sidekick/desktop/"
              ),
          },
        ],
      },
...
const getURL = (route) => {
  if (isDevelopment) {
    ...
    // return `http://0.0.0.0:${process.env.ELECTRON_WEBPACK_WDS_PORT}/?route=${route}`;
  }
  return formatUrl({
...
const createExtensionsWindow = (editorWebContents) => {
  const window = createWindow(
    `sidekick-extensions://./index.html?editor_id=${editorWebContents.id}`,
    {
...
app.on("web-contents-created", (event, webContents) => {
  ...
  webContents.on("will-navigate", (navigateEvent, url) => {
    // !!!!HERE!!!!
    // !!! CHANGE !!!
    if (url === "mailto:contact@mixality.de") {
