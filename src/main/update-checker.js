...
const canUpdateCheckerBeEnabled = () => {
  if (process.env.SIDEKICK_DISABLE_UPDATE_CHECKER) {
    return false;
  }
  // Note: This value is replaced at build-time
  return !!process.env.SIDEKICK_ENABLE_UPDATE_CHECKER;
};
  ...
  //   new Promise((resolve, reject) => {
  {
    return new Promise((resolve, reject) => {
      const request = net.request(
        // !!! CHANGE !!!
        // !!!!!HERE!!!!!
        // "https://mixality.github.io/Sidekick/desktop/version.json"
        "https://menersar.github.io/Sidekick/desktop/version.json"
      );
      request.on("response", (response) => {
        ...
        response.on("end", () => {
          try {
            ...
            log(
              `latest is ${latest}, oldest safe is ${oldestSafe}, current is ${version}`
            );
            resolve({
              latest,
              oldestSafe,
...
// !!! CHANGE !!!
// !!!!!HERE!!!!!
// const getUpdateURL = (current, latest) => `https://mixality.github.io/Sidekick/desktop/update_available.html?from=${encodeURIComponent(current)}&to=${encodeURIComponent(latest)}`;
const getUpdateURL = (current, latest) =>
  // !!! Change back to the following code line? ???
  //   `https://menersar.github.io/Sidekick/desktop/update_available.html?from=${encodeURIComponent(
  {
    return `https://menersar.github.io/Sidekick/desktop/update_available.html?from=${encodeURIComponent(
      current
    )}&to=${encodeURIComponent(latest)}`;
  };
...
const updateAvailable = async (latestVersion) => {
  ...
  const choice = await dialog.showMessageBox(BrowserWindow.getFocusedWindow(), {
    ...
    cancelId: 1,
    message: getTranslation("updater.message").replace(
      "{version}",
      latestVersion
    ),
...
const urgentUpdateAvailable = (latestVersion) => {
  const choice = dialog.showMessageBoxSync(BrowserWindow.getFocusedWindow(), {
    ...
    cancelId: 1,
    message: getTranslation("updater.security.message").replace(
      "{version}",
      latestVersion
    ),
...
const checkForUpdate = () => {
  ...
  //   !!! Calculation of 'lastCheckedThreshold'? ???
